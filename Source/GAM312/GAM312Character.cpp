...
void AGAM312Character::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	//Bind raycast event
	PlayerInputComponent->BindAction("Raycast", IE_Pressed, this, &AGAM312Character::PerformRaycast);
	...
	//Bind action key to camera switch
	PlayerInputComponent->BindAction("Action", IE_Pressed, this, &AGAM312Character::OnAction);
...
void AGAM312Character::OnFire()
{
	...
	this->GetActorEyesViewPoint(TraceStartLocation, TraceStartRotation);

	FVector TraceEnd = TraceStartLocation + (TraceStartRotation.Vector() * 15000);
	...
	DrawDebugLine(GetWorld(), TraceStartLocation, TraceEnd, FColor::Green, false, 1.0f, 0, 1.0f);
	*/
...
/*
FVector AGAM312Character::GetPawnViewLocation() const
{
...
FText AGAM312Character::GetMagicIntText()
{
	...
	FString FullMPS = FString::FromInt(MP);
...
/*
void AGAM312Character::ReceivePointDamage(float Damage, const class UDamageType * DamageType, FVector HitLocation, FVector HitNormal, class UPrimitiveComponent * HitComponent, FName BoneName, FVector ShotFromDirection, class AController * InstigatedBy, AActor * DamageCauser, const FHitResult & HitInfo)
{
	bCanBeDamaged = false;
	redFlash = true;
	UpdateHealth(-Damage);
...
void AGAM312Character::OnAction()
{
	...
	{
		OurPlayer->SetViewTargetWithBlend(MyCamera, 4.f);

		IsFirstPerson = true;
	}
}

void AGAM312Character::PerformRaycast()
{
	...
	{
		GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, FString::Printf(TEXT("You hit nothing")));
	}
